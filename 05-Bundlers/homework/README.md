...
Antes de que vayamos mucho mas adelante, es importante entender el concepto de expresiones de funciones invocadas inmediatamente en Javascript (aka "IIFE"s).
...
Segundo, nota que cuando guardas una función en una variable, esa función también es una expresión de Javascript - una expresión de función. Esto es opuesto a una declaración de función, que es el termino de cuando escribimos funciones usando el keyword `function` sin el `var`.
...
Una vez que la expresión de función es guardada dentro de una variable, podemos por supuesto invocarla:
...
Sin embargo, si envolvés una expresión de función dentro de un par de paréntesis, eso va a causar que la función sea evaluada de la misma forma que sería evaluada si estuviésemos guardándola en una variable. Esto significa que podemos invocar la expresión de función que contiene!
